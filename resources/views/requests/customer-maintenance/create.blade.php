...
<div class="container-fluid px-6 py-4">
    ...
    <form action="{{ route('requests.customer-maintenance.store') }}" method="POST" class="bg-white rounded-xl shadow-md p-6">
                ...
                <div class="mb-6 border-b border-gray-200 pb-4">
                    ...
                    <input type="text" name="request_date" id="request_date" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 date-input" value="{{ old('request_date', $request->request_date ? $request->request_date->format('d/m/Y') : date('d/m/Y')) }}">
                        </div>
                    </div>
                </div>
...
<style>
    ...
    function loadProjectItems(projectId) {
        ...
        fetch(`/api/projects/${projectId}/items`)
            .then(response => response.json())
            .then(items => {
                console.log("Dữ liệu thiết bị từ dự án:", items); // Debug: Kiểm tra dữ liệu trả về
    ...
    function loadRentalItems(rentalId) {
        ...
        fetch(`/api/rentals/${rentalId}/items`)
            .then(response => response.json())
            .then(items => {
                console.log("Dữ liệu thiết bị từ đơn thuê:", items); // Debug: Kiểm tra dữ liệu trả về
