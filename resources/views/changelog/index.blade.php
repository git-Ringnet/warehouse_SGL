        ...
        <main class="p-6">
            ...
            <div class="bg-white rounded-xl shadow-md p-6 border border-gray-100 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                            <label for="change_type" class="block text-sm font-medium text-gray-700 mb-1">Loại hình</label>
                            <select name="change_type" id="change_type" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                <option value="">Tất cả</option>
                                @foreach(\App\Models\ChangeLog::CHANGE_TYPES as $key => $label)
                                    <option value="{{ $key }}" {{ request('change_type') == $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                            ...
                            <input type="date" name="start_date" id="start_date" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" value="{{ request('start_date') }}">
                    </div>
                    <div>
                            <label for="end_date" class="block text-sm font-medium text-gray-700 mb-1">Đến ngày</label>
                            <input type="date" name="end_date" id="end_date" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" value="{{ request('end_date') }}">
                    </div>
                    <div>
                            <label for="search" class="block text-sm font-medium text-gray-700 mb-1">Tìm kiếm</label>
                            <input type="text" name="search" id="search" placeholder="Mã vật tư hoặc tên..." class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" value="{{ request('search') }}">
                    </div>
                </div>

                    <div class="mt-4 flex justify-end space-x-2">
                        <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg transition-colors flex items-center">
                            <i class="fas fa-search mr-2"></i> Lọc
                    </button>
                        <a href="{{ route('change-logs.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg transition-colors flex items-center">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">
                                    <button onclick="showDetails({{ $changeLog->id }})" class="w-8 h-8 flex items-center justify-center rounded-full bg-blue-100 hover:bg-blue-500 transition-colors group" title="Xem chi tiết">
        ...
        function showDetails(id) {
            fetch(`/change-logs/${id}/details`)
                ...
                .catch(error => {
                    ...
                    document.getElementById('modal-content').innerHTML = '<p class="text-red-500">Có lỗi xảy ra khi tải dữ liệu.</p>';
                    detailModal.classList.remove('hidden');
                });
        }
