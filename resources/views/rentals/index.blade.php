...
<head>
    ...
    <title>Quản lý cho thuê - WMS</title>
    ...
    <div class="content-area">
        @php
            ...
            $canCreate =
                $user &&
                ($user->role === 'admin' ||
                    ($user->role_id && $user->roleGroup && $user->roleGroup->hasPermission('rentals.create')));
            $canViewDetail =
                $user &&
                ($user->role === 'admin' ||
                    ($user->role_id && $user->roleGroup && $user->roleGroup->hasPermission('rentals.view_detail')));
            $canEdit =
                $user &&
                ($user->role === 'admin' ||
                    ($user->role_id && $user->roleGroup && $user->roleGroup->hasPermission('rentals.edit')));
            $canDelete =
                $user &&
                ($user->role === 'admin' ||
                    ($user->role_id && $user->roleGroup && $user->roleGroup->hasPermission('rentals.delete')));
        @endphp
        ...
        <header
            ...
            <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 w-full md:w-auto">
                <form action="{{ route('rentals.index') }}" method="GET" class="flex gap-2 w-full md:w-auto">
                    <input type="text" name="search" placeholder="Tìm kiếm theo mã, tên, khách hàng..."
                    ...
                    <select name="filter"
                        ...
                        <option value="rental_code" {{ isset($filter) && $filter == 'rental_code' ? 'selected' : '' }}>
                            Mã phiếu</option>
                        <option value="rental_name" {{ isset($filter) && $filter == 'rental_name' ? 'selected' : '' }}>
                            Tên phiếu</option>
        ...
        <main class="p-6">
                        ...
                        @forelse($rentals as $rental)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap flex space-x-2">
                                    ...
                                    @if ($canDelete)
                                        <button type="button"
                                            onclick="openDeleteModal('{{ $rental->id }}', '{{ $rental->rental_code }}')"
            ...
            <div class="mt-6 flex justify-between items-center">
                <div class="text-sm text-gray-500">
                    Hiển thị {{ $rentals->firstItem() ?? 0 }}-{{ $rentals->lastItem() ?? 0 }} của
                    {{ $rentals->total() }} phiếu cho thuê
                </div>
                <div>
                    {{ $rentals->links() }}
                </div>
