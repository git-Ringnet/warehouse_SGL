...
<head>
    ...
    <title>Chỉnh sửa phiếu cho thuê - WMS</title>
...
<body>
    ...
    <div class="content-area">
        <header class="bg-white shadow-sm py-4 px-6 flex justify-between items-center sticky top-0 z-40">
            ...
            <a href="{{ route('rentals.show', $rental->id) }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 h-10 px-4 rounded-lg flex items-center transition-colors">
                <i class="fas fa-arrow-left mr-2"></i> Quay lại
            </a>
        </header>
        
        <main class="p-6">
            <form action="{{ route('rentals.update', $rental->id) }}" method="POST" class="bg-white rounded-xl shadow-md p-6">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="rental_code" id="rental_code" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-100" value="{{ $rental->rental_code }}" readonly>
                    </div>

                    <!-- Tên phiếu cho thuê -->
                    <div>
                        <label for="rental_name" class="block text-sm font-medium text-gray-700 mb-1 required">Tên phiếu cho thuê</label>
                        <input type="text" name="rental_name" id="rental_name" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ old('rental_name', $rental->rental_name) }}">
                        @error('rental_name')
                    ...
                    <div>
                        ...
                        <div id="customer_info" class="p-3 border border-gray-200 rounded-lg bg-gray-50">
                            <p class="text-sm text-gray-500">Thông tin người đại diện sẽ hiển thị ở đây</p>
                        </div>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="notes" id="notes" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('notes', $rental->notes) }}</textarea>
                        @error('notes')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    <a href="{{ route('rentals.show', $rental->id) }}" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">
                        Hủy
                    </a>
    ...
    <script>
        ...
        // Khởi tạo thông tin khách hàng khi tải trang
