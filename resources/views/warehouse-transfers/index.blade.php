    ...
    <div class="content-area">
        <header class="bg-white shadow-sm py-4 px-6 flex flex-col md:flex-row md:justify-between md:items-center sticky top-0 z-40 gap-4">
            ...
            <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 w-full md:w-auto">
                ...
                <a href="{{ route('warehouse-transfers.create') }}" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center transition-colors w-full md:w-auto justify-center">
        ...
        <main class="p-6">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $transfer->transfer_date->format('d/m/Y') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $transfer->employee->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 py-1 {{ $transfer->status_class }} rounded-full text-xs font-semibold">
                                    {{ $transfer->status_label }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $transfer->notes ?? '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap flex space-x-2">
                                <a href="{{ route('warehouse-transfers.show', $transfer->id) }}" class="w-8 h-8 flex items-center justify-center rounded-full bg-blue-100 hover:bg-blue-500 transition-colors group" title="Xem">
                                    <i class="fas fa-eye text-blue-500 group-hover:text-white"></i>
                                </a>
                                <a href="{{ route('warehouse-transfers.edit', $transfer->id) }}" class="w-8 h-8 flex items-center justify-center rounded-full bg-yellow-100 hover:bg-yellow-500 transition-colors group" title="Sửa">
                                    <i class="fas fa-edit text-yellow-500 group-hover:text-white"></i>
                                </a>
                                <button onclick="openDeleteModal('{{ $transfer->id }}', '{{ $transfer->transfer_code }}')" 
                                ...
                                <form id="delete-form-{{ $transfer->id }}" action="{{ route('warehouse-transfers.destroy', $transfer->id) }}" method="POST" class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </td>
            ...
            <div class="mt-4 flex justify-between items-center">
                <div class="text-sm text-gray-600">
                    ...
                    Hiển thị {{ $warehouseTransfers->firstItem() }} đến {{ $warehouseTransfers->lastItem() }} của {{ $warehouseTransfers->total() }} bản ghi
                    @else
