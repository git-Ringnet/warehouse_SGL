        ...
        <header class="bg-white shadow-sm py-4 px-6 flex flex-col md:flex-row md:justify-between md:items-center sticky top-0 z-40 gap-4">
            ...
            <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 w-full md:w-auto">
                <form action="{{ route('projects.index') }}" method="GET" class="flex gap-2 w-full md:w-auto">
                    <input type="text" name="search" placeholder="Tìm kiếm theo tên, mã dự án..." class="border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50 text-gray-700 w-full md:w-64 h-10" value="{{ $search ?? '' }}" />
                    <select name="filter" class="border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50 text-gray-700 h-10">
                        ...
                        <option value="project_code" {{ isset($filter) && $filter == 'project_code' ? 'selected' : '' }}>Mã dự án</option>
                        <option value="project_name" {{ isset($filter) && $filter == 'project_name' ? 'selected' : '' }}>Tên dự án</option>
                ...
                <a href="{{ route('projects.create') }}" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center transition-colors w-full md:w-auto justify-center h-10">
        ...
        <main class="p-6">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $project->project_name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $project->customer->company_name }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap flex space-x-2">
                                ...
                                <a href="{{ route('projects.edit', $project->id) }}" class="w-8 h-8 flex items-center justify-center rounded-full bg-yellow-100 hover:bg-yellow-500 transition-colors group" title="Sửa">
                                    <i class="fas fa-edit text-yellow-500 group-hover:text-white"></i>
                                </a>
                                <button type="button" onclick="openDeleteModal('{{ $project->id }}', '{{ $project->project_name }}')" class="w-8 h-8 flex items-center justify-center rounded-full bg-red-100 hover:bg-red-500 transition-colors group" title="Xóa">
            ...
            <div class="mt-6 flex justify-between items-center">
                <div class="text-sm text-gray-500">
                    Hiển thị {{ $projects->firstItem() ?? 0 }}-{{ $projects->lastItem() ?? 0 }} của {{ $projects->total() }} dự án
                </div>
                <div>
                    {{ $projects->links() }}
                </div>
