...
<body>
    ...
    <div class="content-area">
        ...
        <main class="p-6">
            <form action="{{ route('projects.store') }}" method="POST" class="bg-white rounded-xl shadow-md p-6">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="start_date" id="start_date" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ old('start_date') ?? date('Y-m-d') }}">
                    </div>

                    <!-- Ngày kết thúc dự kiến -->
                    <div>
                        <label for="end_date" class="block text-sm font-medium text-gray-700 mb-1 required">Ngày kết thúc dự kiến</label>
                        <input type="date" name="end_date" id="end_date" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ old('end_date') ?? date('Y-m-d', strtotime('+1 month')) }}">
                    </div>

                    <!-- Thời gian bảo hành (tháng) -->
                    <div>
                        <label for="warranty_period" class="block text-sm font-medium text-gray-700 mb-1 required">Thời gian bảo hành (tháng)</label>
                        <input type="number" name="warranty_period" id="warranty_period" required min="1" max="36" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ old('warranty_period') ?? 12 }}">
                    </div>
    ...
    <script>
        ...
        // Nếu đã có khách hàng được chọn từ trước (ví dụ khi validation fail), hiển thị thông tin
